...
<?php
function table_start($building, $status){
    if ($status == "Submitted"){
     $color = green;
     $section = s;
...
include "Config.php";
$sql0 = "SELECT COUNT(*) FROM WORKORDER WHERE WO_STATUS = '$status' AND WO_SCHOOL = '$building'";
...
$sql = "SELECT * FROM WORKORDER NATURAL JOIN USER WHERE WO_SCHOOL = '$building' AND WO_STATUS= '$status' ORDER BY WO_ID DESC LIMIT $start_from, ".$results_per_page;
...
<?php
  ...
  ?>
<?php
$sql = "SELECT COUNT(*) AS total FROM WORKORDER WHERE WO_STATUS = '$status' AND WO_SCHOOL = '$building'";
$result = $db->query($sql);
