...
<?php
function table_start($user1, $status){
    if ($status == "Submitted"){
     $color = green;
     $section = s;
...
include "Config.php";
$sql0 = "SELECT COUNT(*) FROM WORKORDER WHERE WO_STATUS = '$status' AND USER_ID = '$user1'";
...
$sql = "SELECT * FROM WORKORDER NATURAL JOIN USER WHERE USER_ID = '$user1' AND WO_STATUS= '$status' ORDER BY WO_ID DESC LIMIT $start_from, ".$results_per_page;
...
<?php
  ...
  ?>
<?php
$sql = "SELECT COUNT(*) AS total FROM WORKORDER WHERE WO_STATUS = '$status' AND USER_ID = '$user1'";
$result = $db->query($sql);
