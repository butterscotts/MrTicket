...
$sql = "SELECT * FROM WORKORDER NATURAL JOIN USER WHERE WO_SCHOOL = '$building' AND (WO_STATUS = 'Submitted' OR WO_STATUS ='Completed') ORDER BY WO_ID DESC LIMIT $start_from, ".$results_per_page;
...
<?php
$sql = "SELECT COUNT(*) AS total FROM WORKORDER WHERE WO_SCHOOL = '$building' AND (WO_STATUS = 'Submitted' OR WO_STATUS ='Completed')";
$result = $db->query($sql);
