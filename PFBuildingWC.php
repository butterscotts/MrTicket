...
<?php
function table_start($building, $statusS, $statusW){
include 'Config.php';
$results_per_page = 10;
if (isset($_GET["page".$section.""])) { $page  = $_GET["page".$section.""]; } else { $page=1; }; 
$start_from = ($page-1) * $results_per_page;
$sql = "SELECT * FROM WORKORDER NATURAL JOIN USER WHERE WO_SCHOOL = '$building' AND (WO_STATUS = 'Work In Progress' OR WO_STATUS ='Completed') ORDER BY WO_ID DESC LIMIT $start_from, ".$results_per_page;
...
<?php
$sql = "SELECT COUNT(*) AS total FROM WORKORDER WHERE WO_SCHOOL = '$building' AND (WO_STATUS = 'Work In Progress' OR WO_STATUS ='Completed')";
$result = $db->query($sql);
