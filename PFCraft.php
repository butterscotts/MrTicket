...
<?php
function table_start($craft, $status){
    if ($status == "Submitted"){
     $color = green;
     $section = s;
...
include "Config.php";
$sql0 = "SELECT COUNT(*) FROM WORKORDER WHERE WO_STATUS = '$status' AND WO_CRAFT = '$craft'";
...
$sql = "SELECT * FROM WORKORDER NATURAL JOIN USER WHERE WO_CRAFT = '$craft' AND WO_STATUS= '$status' ORDER BY WO_ID DESC LIMIT $start_from, ".$results_per_page;
...
<div class="content">
 ...
 <tr>
    ...
    <th style="background-color:<?php echo $color ?>">Requested By</th>
...
<?php 
 while($row = $rs_result->fetch_assoc()) {
      echo "<tr>";
      echo "<td style=color:blue><a href=Edit.1.php?wo=".$row['WO_ID'].">Edit Ticket<br> #".$row['WO_ID']."</td>";
...
<?php
  ...
  ?>
<?php
$sql = "SELECT COUNT(*) AS total FROM WORKORDER WHERE WO_STATUS = '$status' AND WO_CRAFT = '$craft'";
$result = $db->query($sql);
$row = $result->fetch_assoc();
$total_pages = ceil($row["total"] / $results_per_page); // calculate total pages with results
echo "Showing Page #".$page." of the ".$status." table <br>";
for ($i=1; $i<=$total_pages; $i++) {  // print links for all pages
            echo "<a href='/Pages/SearchWindow.1.php?page".$section."=".$i."?var=".$craft."'";
